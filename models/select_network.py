...
def define_G(opt):
    ...
    elif net_type == 'attnunet':
        from models.network_attnunet import Restormer as net
        netG = net(inp_channels=opt_net['in_nc'],
                   out_channels=opt_net['out_nc'],
                   dim=opt_net['dim'],
                   num_blocks=opt_net['num_blocks'],
    ...
    # ----------------------------------------
    ...
    elif net_type == 'chasnetx2':
        from models.network_chasnetx2 import VKGen as net
        netG = net(in_nc=opt_net['in_nc'],
                   nf=opt_net['nf'])

    elif net_type == 'chasnetx4':
        from models.network_chasnetx4 import VKGen as net
        netG = net(in_nc=opt_net['in_nc'],
                   nf=opt_net['nf'])

    elif net_type == 'rcan':
        from models.network_rcan import RCAN as net
        netG = net(n_resgroups=opt_net['n_resgroups'],
        ...
        netG = net(G0=opt_net['G0'],
                   RDNkSize=opt_net['RDNkSize'],
                   RDNconfig=opt_net['RDNconfig'],
                   scale=opt_net['scale'],
                   img_range=opt_net['img_range'],
                   in_chans=opt_net['in_chans'])

    elif net_type == 'nlsn':
        from models.network_nlsn import NLSN as net
        netG = net(n_resblocks=opt_net['n_resblocks'],
                   n_feats=opt_net['n_feats'],
                   chunk_size=opt_net['chunk_size'],
                   n_hashes=opt_net['n_hashes'],
                   scale=opt_net['scale'],
                   img_range=opt_net['img_range'],
                   in_chans=opt_net['in_chans'])
    ...
    elif net_type == 'grl_denoising':
        from GRL.network_grl import GRL as net
        netG = net(
            ...
            depths=opt_net['depths'],
            embed_dim=opt_net['embed_dim'],
            num_heads_window=opt_net['num_heads_window'],
            num_heads_stripe=opt_net['num_heads_stripe'],
            mlp_ratio=opt_net['mlp_ratio'],
            qkv_proj_type=opt_net['qkv_proj_type'],
            anchor_proj_type=opt_net['anchor_proj_type'],
            anchor_window_down_factor=opt_net['anchor_window_down_factor'],
            out_proj_type=opt_net['out_proj_type'],
            conv_type=opt_net['conv_type'],
            upsampler="",
            local_connection=True)

    elif net_type == 'grl_ffn_denoising':
        from GRL_ffn.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_frec_denoising':
        from GRL_ffn_frec.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_frea_denoising':
        from GRL_ffn_frea.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_flatten_denoising':
        from GRL_ffn_flatten.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_lsk_denoising':
        from GRL_ffn_lsk.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_fre_denoising':
        from GRL_ffn_fre.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_slak_denoising':
        from GRL_ffn_slak.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_ffn_coordatt_denoising':
        from GRL_ffn_coordatt.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'grl_szw_denoising':
        from GRL_szw.network_grl import GRL as net
        netG = net(
            in_channels=opt_net['in_channels'],
            upscale=1,
            img_size=opt_net['img_size'],
    ...
    elif net_type == 'art_denoising':
        from models.network_art import ART as net
        netG = net(
    ...
    elif net_type == 'rednet':
        from models.network_rednet import REDNet30 as net
        netG = net()

    elif net_type == 'panet':
        from models.network_panet import PANET as net
        netG = net()

    elif net_type == 'memnet':
        from models.network_memnet import MemNet as net
        netG = net()

    elif net_type == 'drunet':
        from models.network_drunet import UNetRes as net
        netG = net(in_nc=1, out_nc=1, nc=[64, 128, 256, 512],
                   nb=4, act_mode='R', downsample_mode="strideconv",
                   upsample_mode="convtranspose")

    elif net_type == 'dagl':
        from models.network_dagl import RR as net
        netG = net()

    elif net_type == 'mwcnn':
        from models.network_mwcnn import BSR as net
        netG = net()

    elif net_type == 'hinet':
        from models.network_hinet import HINet as net
        netG = net()
