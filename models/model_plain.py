...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        if G_lossfn_type == 'l1':
            ...
            self.G_lossfn = CharbonnierLoss(eps=1e-5).to(self.device)
        else:
    ...
    def feed_data_B(self, data, need_H=True):
        self.L = data['L'].to(self.device)
        self.B = data['B'].to(self.device)
        if need_H:
            self.H = data['H'].to(self.device)
        # print(self.L.shape, self.H.shape, self.B.shape)
    # ----------------------------------------
    # feed L to netG
    # ----------------------------------------
    ...
    def netG_forward_H(self):
        self.E = self.netG(self.L)[1]

    # ----------------------------------------
    # feed L to netG
    # ----------------------------------------
    def netG_forward_B(self):
        # self.E = self.netG(self.L, self.B)
        self.E = self.netG(self.L, self.B)
    ...
    # update parameters and get loss
    # ----------------------------------------
    def optimize_parameters_B(self, current_step):
        self.G_optimizer.zero_grad()
        self.netG_forward_B()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.E, self.H)
        G_loss.backward()
    ...
    def optimize_parameters_H(self, current_step):
        self.G_optimizer.zero_grad()
        self.netG_forward_H()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.E, self.H)
        G_loss.backward()
    ...
    def test_B(self):
        self.netG.eval()
        # print(self.L.shape, self.B.shape, self.H.shape)
        with torch.no_grad():
            self.netG_forward_B()
        self.netG.train()
